...
import TableSampleCountries from '@/components/Countries/TableSampleCountries.vue'
...
const deleteFilter = (index) => {
  ...
  generateQuery();
  countriesStore.fetch(null, query.value);
}
...
const externalLink = computed(() => {
  return process.env.NODE_ENV === 'production'
    ? window.location.origin + '/api-docs/#/Countries'
    : 'http://localhost:8080/api-docs/#/Countries';
});
...
<template>
  ...
  <SectionHeroBar>Countries</SectionHeroBar>
  <SectionMain>
    <BaseButtons>
      <BaseButton
        color="default"
  ...
  <SectionMain>
  <a class="text-blue-500 hover:text-blue-600 active:text-blue-700 mb-3 block" :href="externalLink">API documentation for countries</a>
    ...
    <CardBox
      ...
      has-table
    >
      <TableSampleCountries checkable :sortTitle="sortTitle" :sortDirection="sortDirection" @sort="sortTable($event)" />
    </CardBox>

...
export const useCountriesStore = defineStore('countries', {
  ...
  actions: {
    ...
    async newItem(payload) {
      this.startLoading();
      try {
        const result = await axios.post('/countries', { data: payload });
        this.showNotification('Countries has been created', 'success');
    ...
    async edit(payload) {
      this.startLoading();
      try {
        const result = await axios.put(`/countries/${payload.id}`, {id: payload.id, data: payload.data})
        // 
        this.showNotification('Countries has been updated', 'success');
        this.getData(result.data)
      } catch (e) {
